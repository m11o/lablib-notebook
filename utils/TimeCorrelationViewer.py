...
class TimeCorrelationViewer:
    ...
    def view_heatmap(self):
        ...
        ax_for_all = sns.heatmap(self.shuffle_df, ax=axes[0], robust=True, vmin=self.SCALE_MIN, vmax=self.SCALE_MAX, square=True, cmap=self.HEATMAP_CMAP)
        ax_for_all.invert_yaxis()
        ax_for_engram = sns.heatmap(self.engram_df, ax=axes[1], robust=True, vmin=self.SCALE_MIN, vmax=self.SCALE_MAX, square=True, cmap=self.HEATMAP_CMAP)
        ax_for_engram.invert_yaxis()
        ax_for_non_engram = sns.heatmap(self.non_engram_df, ax=axes[2], robust=True, vmin=self.SCALE_MIN, vmax=self.SCALE_MAX, square=True, cmap=self.HEATMAP_CMAP)
    ...
    def view_sum_of_correlation_bar(self):
        ...
        axes[0][0].plot(shuffle_mean, label='all cells')
        axes[0][0].plot(engram_mean, label='engram cells')
        axes[0][0].plot(non_engram_mean, label='non-engram cells')
        axes[0][0].legend()
        ...
        axes[0][1].plot(abs_shuffle_sum, label='all cells')
        axes[0][1].plot(abs_engram_sum, label='engram cells')
        axes[0][1].plot(abs_non_engram_sum, label='non-engram cells')
        axes[0][1].legend()

        sns.distplot(abs_engram_sum, bins=5, label='engram cells', ax=axes[1][0])
        sns.distplot(abs_non_engram_sum, bins=5, label='non-engram cells', ax=axes[1][0])
        axes[1][0].legend()

        engram_sum_df = pd.DataFrame(abs_engram_sum).assign(Cell='engram cells')
        non_engram_sum_df = pd.DataFrame(abs_non_engram_sum).assign(Cell='non-engram cells')
    ...
    def scale_df(shuffle_df, engram_df, non_engram_df):
        ...
        shuffle_df = scaler.transform(shuffle_df)
        shuffle_df = pd.DataFrame(shuffle_df.astype(float), columns=list(range(seconds)), index=list(range(seconds)))
        engram_df = scaler.transform(engram_df)
        engram_df = pd.DataFrame(engram_df.astype(float), columns=list(range(seconds)), index=list(range(seconds)))
        non_engram_df = scaler.transform(non_engram_df)
        non_engram_df = pd.DataFrame(non_engram_df.astype(float), columns=list(range(seconds)), index=list(range(seconds)))

        return [shuffle_df, engram_df, non_engram_df]
